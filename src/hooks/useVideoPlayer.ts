...
// ELIMINADO: VideoJSSubtitleSyncPlugin - El botón de configuración ahora se agrega directamente en useVideoPlayer
...
interface UseVideoPlayerOptions {
  ...
  // Audio personalizado (español latino y English Dub)
  customStreamUrl?: string | null; // URL del stream personalizado (latino)
  englishDubStreamUrl?: string | null; // URL del stream en inglés doblado
...
export function useVideoPlayer({ streamUrl, videoDuration, movieTitle, moviePoster, onError, onReady, onClose, onEpisodeButtonClick, showEpisodeButton, onNextEpisodeClick, showNextEpisodeButton, onTimeUpdate, imdbId, tmdbId, season, episode, episodeTitle, isTV, movieHash, availableSubtitles = [], getAvailableSubtitles, subdivxResults = [], onSubdivxResultsChange, logoPath, year, rating, overview, customStreamUrl, englishDubStreamUrl, onToggleAudioMenu }: UseVideoPlayerOptions) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const playerRef = useRef<ReturnType<typeof videojs> | null>(null);
  const hlsRef = useRef<Hls | null>(null); // ⚡ Instancia de hls.js
  ...
  // 🔥 Ref para siempre tener el estado más reciente de subtitleSettings
  const subtitleSettingsRef = useRef(playerState.subtitleSettings);
  useEffect(() => {
    subtitleSettingsRef.current = playerState.subtitleSettings;
  }, [playerState.subtitleSettings]);
  ...
  const addSubtitle = useCallback((file: File, language: string = 'es') => {
    ...
    try {
      // 🎨 Detectar si es formato ASS/SSA por el nombre del archivo
      const isASS = file.name.toLowerCase().endsWith('.ass') || 
                    file.name.toLowerCase().endsWith('.ssa');
      
      if (isASS) {
        playerLogger.log(`🎨 [SUBTITLES] Archivo ASS/SSA detectado: ${file.name}`);
        ...
        reader.onload = (e) => {
          ...
          if (content) {
            playerLogger.log('🎨 [SUBTITLES] Emitiendo evento ass-subtitle-available');
            
            // Emitir evento para que el componente lo maneje
  ...
  const openSubtitleSettings = useCallback(() => {
    ...
    const downloadAndLoadOpenSubtitle = async (subtitle: any) => {
      ...
      try {
        ...
        if (subtitle.vtt) {
          ...
          playerLogger.log('🔍 [WYZIE-DEBUG] Verificando subtítulo:', {
            hasVTT: !!vttContent,
            vttLength: vttContent?.length,
            format: subtitle.format,
            isASS: subtitle.isASS,
            firstChars: vttContent?.substring(0, 100)
          });
          ...
          const isASS = vttContent.includes('[Script Info]') || 
                       ...
                       subtitle.format === 'ass' ||
                       subtitle.format === 'ssa' || // ← Agregar SSA
                       subtitle.isASS === true;
          
          playerLogger.log('🔍 [WYZIE-DEBUG] Resultado detección ASS:', isASS);
          
          if (isASS) {
            playerLogger.log('🎨 [WYZIE] Subtítulo ASS/SSA detectado - NO agregando al player nativo');
            playerLogger.log('🎨 [WYZIE] El subtítulo se renderizará con ASS.js externamente');
            ...
            const event = new CustomEvent('ass-subtitle-available', {
              detail: {
                ...
                label: `${subtitle.display || subtitle.language} (${subtitle.media})`,
              }
            });
            window.dispatchEvent(event);
        ...
        if (subtitle.url && subtitle.url.includes('wyzie.ru')) {
          ...
          // 🎨 DETECTAR SI ES ASS/SSA
          const isASS = srtContent.trim().startsWith('[Script Info]') || 
                       subtitle.url.includes('format=ssa') || 
                       subtitle.url.includes('format=ass');
          
          if (isASS) {
            playerLogger.log('🎨 [WYZIE] Detectado ASS/SSA, NO convirtiendo');
            
            // Crear VTT dummy para que Video.js no arroje error
            ...
            // Guardar contenido ASS en el track
            if (track && track.track) {
              (track.track as any).assContent = srtContent;
              (track.track as any).isASS = true;
            }
            
            playerLogger.log('✅ [WYZIE-ASS] Track ASS agregado con VTT dummy:', label);
          } else {
          // Convertir SRT a VTT
      ...
      Object.entries(groupedByLanguage).forEach(([langName, tracks]) => {
        ...
        if (tracks.length === 1) {
          groupButton.onclick = (e) => {
            ...
            const assContent = (track as any).assContent;
            ...
            if (isASS && assContent) {
              ...
              // 🎨 Desactivar cualquier ASS que esté activo
              playerLogger.log(`📝 [MODAL] Subtítulo VTT seleccionado, desactivando ASS si está activo`);
              const deactivateEvent = new CustomEvent('ass-subtitle-deactivate');
              window.dispatchEvent(deactivateEvent);
            }
            
            playerLogger.log(`✅ [MODAL] Subtítulo seleccionado (grupo único): ${track.label}`);
          ...
          tracks.forEach((track: any, index: number) => {
            ...
            trackItem.onclick = (e) => {
              ...
              playerLogger.log(`✅ [MODAL] Subtítulo seleccionado: ${track.label}`);
    ...
    const renderSettingsTab = () => {
      // 🔥 Usar el ref para obtener SIEMPRE el estado más reciente
      const currentSettings = subtitleSettingsRef.current;
      
      // 🔍 LOG PARA DEBUGGING
      playerLogger.log('🎛️ [SETTINGS-TAB] Configuración actual al abrir:', currentSettings);
      
      // 🔥 Variables locales que se actualizan con cada cambio
      ...
      const createOffsetButton = (text: string, delta: number) => {
        ...
        btn.onclick = (e) => {
          ...
          offset += delta;
          offsetDisplay.textContent = `${offset >= 0 ? '+' : ''}${offset.toFixed(1)}s`;
          // 🔥 Usar todas las variables locales actualizadas
          applySubtitleSettings({ offset, fontPercent, textColor, backgroundColor, backgroundOpacity, fontFamily, position });
        };
        return btn;
      };
  ...
  }, [applySubtitleSettings]); // ✅ Ya no depende de playerState.subtitleSettings
  ...
  useEffect(() => {
    ...
    // Si ya hay un player existente y válido, solo cambiar la source (MUY RÁPIDO, sin pantalla negra)
    if (playerRef.current && !playerRef.current.isDisposed()) {
      try {
        playerLogger.log('🔄 [PLAYER] Player ya existe, cambiando source directamente...');
        const player = playerRef.current;
        const currentTime = (window as any).resumeTime || 0;
        
        // Determinar el tipo de video con detección agresiva de HLS
        const urlLower = streamUrl.toLowerCase();
        const isHLS = urlLower.includes('.m3u8') || urlLower.includes('.txt') || urlLower.includes('.woff2');
        const videoType = isHLS ? 'application/x-mpegURL' : 'video/mp4';
        
        playerLogger.log(`🎯 [PLAYER] Tipo detectado: ${videoType} para ${streamUrl.substring(streamUrl.lastIndexOf('/') + 1)}`);
        
        // ⚡ Si es HLS, limpiar instancia anterior de hls.js y crear nueva
        if (isHLS && Hls.isSupported()) {
          playerLogger.log('🚀 [HLS.JS] Cambiando source usando hls.js');
          ...
          // Crear nueva instancia optimizada
          const hls = new Hls({
            debug: false,
            enableWorker: true,
          ...
          // No es HLS o no hay soporte, cambiar source normalmente
          player.src({ src: streamUrl, type: videoType });
        }
        
        // Agregar listeners para debugging
        player.one('loadstart', () => {
          playerLogger.log('📥 [PLAYER] loadstart - Comenzando a cargar stream');
        });
        
        player.one('loadedmetadata', () => {
          playerLogger.log(`✅ [PLAYER] loadedmetadata - Metadatos cargados, duración: ${player.duration()}s`);
          if (currentTime > 0) {
            player.currentTime(currentTime);
            delete (window as any).resumeTime;
            playerLogger.log(`⏩ [PLAYER] Resumido en ${currentTime}s`);
          }
        });
        
        // 🚀 CRÍTICO: Intentar play en loadeddata (más temprano que canplay)
        player.one('loadeddata', () => {
          playerLogger.log('✅ [PLAYER] loadeddata - Datos iniciales cargados, intentando autoplay agresivo');
          if (player && typeof player.paused === 'function' && player.paused()) {
            player.play()?.catch((e: any) => {
              playerLogger.warn('⚠️ [PLAYER] Autoplay bloqueado por navegador, esperando interacción:', e);
            });
          }
        });
        
        player.one('canplay', () => {
          playerLogger.log('✅ [PLAYER] canplay - Buffer suficiente');
        });
        
        player.one('playing', () => {
          playerLogger.log('▶️ [PLAYER] playing - Reproducción iniciada');
        });
        
        player.one('error', () => {
          const error = player.error();
          playerLogger.error('❌ [PLAYER] Error de reproducción:', {
            code: error?.code,
        ...
        playerLogger.log(`✅ [PLAYER] Source cambiada exitosamente`);
        setPlayerState(prev => ({ ...prev, isLoading: false }));
        return; // Salir temprano, NO recrear el player
      } catch (error) {
        playerLogger.warn('⚠️ [PLAYER] Error cambiando source, recreando player:', error);
        // Si falla, continuar con la lógica de recreación
      }
    }

    playerLogger.log('🎬 [PLAYER] Creando nuevo reproductor con URL:', streamUrl);
    setPlayerState(prev => ({ ...prev, isLoading: true }));

    // Limpiar reproductor existente si está corrupto o no existe
    if (playerRef.current) {
      try {
        playerRef.current.dispose();
      } catch (error) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const playerOptions: any = {
          ...
          fluid: true,
          autoplay: true,
          preload: 'metadata', // ⚡ Solo metadata inicial, reproducción más rápida
          muted: false, // No silenciado por defecto, el usuario puede ajustar
          playbackRates: [0.5, 1, 1.25, 1.5, 2],
          inactivityTimeout: 3000,
          // 🔧 CRÍTICO: DESACTIVAR userActions nativos porque usamos listeners manuales
          // (Los nativos de Video.js tienen conflictos con el botón de subtítulos)
          userActions: {
            hotkeys: true,
            click: false, // ❌ Desactivado - usamos listener manual
            doubleClick: false, // ❌ Desactivado - usamos listener manual
          },
          ...
          html5: {
            vhs: {
              overrideNative: true,
              // ⚡ CONFIGURACIÓN ANTI-FLUSHING para VHS nativo
              enableLowInitialBitrate: false, // NO forzar calidad baja
              ...
              // 🚀 BUFFER GRANDE ANTI-FLUSHING (igual que HLS.js que probamos)
              backBufferLength: Infinity, // NO limpiar buffer atrás
              maxMaxBufferLength: 600, // 10 minutos máximo
              maxBufferSize: 60 * 1024 * 1024, // 60 MB en buffer
              maxBufferHole: Infinity, // NO intentar llenar huecos (evita seeks)
              xhr: {
          ...
          controlBar: {
            children: [
              ...
              // NO incluir 'subtitlesButton' aquí - lo agregamos manualmente después
              // para evitar que Video.js cree también el botón default (SubsCapsButton)
              'chromecastButton',
              'fullscreenToggle',
            ],
          },
        ...
        // 🎯 NUEVO: Registrar botón de configuración ANTES de crear el player
        // Esto garantiza que siempre esté disponible desde el inicio
        const Button = videojs.getComponent('Button');
        const MenuItem = videojs.getComponent('MenuItem');
        
        // Componente personalizado para el item "Configuración" en el menú de subtítulos
        class SubtitleSettingsMenuItem extends MenuItem {
          constructor(player: any, options: any) {
            super(player, options);
            this.addClass('vjs-subtitle-settings-btn');
          }
          
          handleClick() {
            playerLogger.log('🎛️ [SUBTITLE-SETTINGS] Click en Configuración');
            
            // Cerrar el menú de subtítulos
            const playerAny = this.player() as any;
            const parent = playerAny.controlBar?.getChild('SubtitlesButton') || 
                          playerAny.controlBar?.getChild('SubsCapsButton');
            if (parent && parent.menu) {
              parent.menu.hide();
            }
            
            // Disparar evento para abrir el modal
            window.dispatchEvent(new CustomEvent('openSubtitleSettings'));
          }
        }
        
        // Registrar el componente
        videojs.registerComponent('SubtitleSettingsMenuItem', SubtitleSettingsMenuItem);
        playerLogger.log('✅ [SUBTITLE-SETTINGS] Componente registrado');
        ...
        // 🎯 IMPORTANTE: Agregar el botón de subtítulos manualmente para evitar el botón default
        player.ready(() => {
          const playerAny = player as any;
          const controlBar = playerAny.controlBar;
          
          if (controlBar) {
            // Verificar si ya existe un botón de subtítulos
            let subtitlesButton = controlBar.getChild('SubtitlesButton');
            
            // Si no existe, crearlo manualmente
            if (!subtitlesButton) {
              try {
                const SubtitlesButton = videojs.getComponent('SubtitlesButton');
                subtitlesButton = new SubtitlesButton(player, {});
                
                // Agregar ANTES del botón de chromecast
                const chromecastButton = controlBar.getChild('chromecastButton');
                if (chromecastButton) {
                  const chromecastIndex = controlBar.children().indexOf(chromecastButton);
                  controlBar.addChild(subtitlesButton, {}, chromecastIndex);
                } else {
                  controlBar.addChild(subtitlesButton);
                }
                
                playerLogger.log('✅ [SUBTITLES] Botón creado manualmente');
              } catch (e) {
                playerLogger.error('❌ [SUBTITLES] Error creando botón:', e);
              }
            }
            
            // Agregar el item de configuración al menú
            if (subtitlesButton && subtitlesButton.menu) {
              // Agregar el item personalizado al menú
              const settingsItem = new SubtitleSettingsMenuItem(player, {
                label: 'Configuración',
                selectable: false,
              });
              
              // Agregar al final del menú con un separador visual
              const menuItems = subtitlesButton.menu.children();
              const lastItem = menuItems[menuItems.length - 1];
              if (lastItem) {
                // Agregar borde superior para separar visualmente
                const el = settingsItem.el() as HTMLElement;
                el.style.borderTop = '1px solid rgba(255,255,255,0.1)';
                el.style.marginTop = '4px';
              }
              
              subtitlesButton.menu.addChild(settingsItem);
              playerLogger.log('✅ [SUBTITLE-SETTINGS] Botón agregado al menú de subtítulos');
            } else {
              playerLogger.warn('⚠️ [SUBTITLE-SETTINGS] No se encontró botón de subtítulos');
            }
          }
        });
        
        // ⚡ OPTIMIZACIÓN: Agregar listener de loadstart para debug de HLS
        player.one('loadstart', () => {
          playerLogger.log('📥 [PLAYER] loadstart - VHS comenzó a cargar playlist');
          
          // Intentar acceder al tech de VHS para ver su estado
          const tech = player.tech();
          if (tech && (tech as any).vhs) {
            const vhs = (tech as any).vhs;
            playerLogger.log('🎯 [VHS] Estado:', {
              state: vhs.state,
              playlists: vhs.playlists?.media_?.length || 0,
            });
            
            // Listener para ver cuándo carga segmentos
            vhs.on('progress', () => {
              ...
              if (buffered.length > 0) {
                ...
                playerLogger.log(`⏬ [VHS] Buffer: ${bufferedAhead.toFixed(1)}s adelante`);
              }
            });
          }
        });
        ...
        player.ready(() => {
          ...
          try {
            ...
            if (cb && cb.el()) {
              ...
              let setupRetries = 0; // Contador de reintentos
              const setupPixelPerfectCenter = () => {
                ...
                // ✅ ACTUALIZADO: El botón de subtítulos puede estar oculto (vjs-hidden) si no hay subtítulos disponibles
                // Eso es NORMAL y NO es un error. Solo verificamos que exista en el DOM.
                if (!subsBtn) {
                  setupRetries++;
                  
                  if (setupRetries >= 3) {
                    playerLogger.log('⚠️ [TITLE] Botón de subtítulos no encontrado después de 3 reintentos, continuando de todas formas...');
                    // NO hacer reload, simplemente continuar sin el botón
                    } else {
                    playerLogger.log(`🔄 [TITLE] Botón de subtítulos no encontrado (intento ${setupRetries}/3), reintentando...`);
                    setTimeout(setupPixelPerfectCenter, 300);
                    return;
                  }
                }
                
                playerLogger.log('✅ [TITLE] Botones correctos, continuando...');
                ...
                // Mover botones derecha al grupo (orden correcto: audio, subs, episodios, next, fullscreen)
                const rightButtons = [audioBtn, subsBtn, episodeBtn, nextEpisodeBtn, fullscreenBtn].filter(Boolean);
          ...
          // ELIMINADO: initSubtitleSync() - El botón de configuración ahora se agrega directamente en la inicialización
          // Ya no necesitamos el plugin con setTimeout, el botón es parte del player desde el inicio
        ...
        if (chromecastLoaded) {
          ...
          const initChromecast = () => {
            ...
            player.ready(() => {
              ...
              // NOTA: El botón de Chromecast ahora se maneja igual que el botón Volver
              // Ver código después de la creación del botón Volver (líneas ~4504+)
              } catch (error) {
          ...
          playerLogger.warn('⚠️ [CHROMECAST] Plugin NO cargado');
            ...
            player.ready(() => {
                ...
                (player as any).subtitleUpload({
                  onFileSelected: async (file: File) => {
                    ...
                    try {
                      ...
                      const isASS = text.includes('[Script Info]') || 
                                   text.includes('[V4+ Styles]') || 
                                   text.includes('Format: Layer') ||
                                   file.name.toLowerCase().endsWith('.ass') ||
                                   file.name.toLowerCase().endsWith('.ssa');
                      
                      // 🎨 Si es ASS/SSA, emitir evento y NO procesar
                      if (isASS) {
                        playerLogger.log('🎨 [SUBTITLES] Archivo ASS/SSA detectado, emitiendo evento');
                      ...
                      setTimeout(() => {
                        // ELIMINADO: Ya no necesitamos reinicializar el plugin
                        // El botón de configuración ahora es parte del player desde el inicio
                        playerLogger.log('ℹ️ [SUBTITLES] Botón de configuración ya está presente desde la inicialización');

                        // Reaplicar fix de hover después de cargar subtítulos locales
                        setTimeout(() => {
                          const playerEl = player.el();
                          if (!playerEl) return;
              ...
              const getVideoType = (url: string): string | undefined => {
                ...
                // ✅ Detectar streams HLS de forma MUY agresiva
                // CUALQUIER archivo .m3u8, .txt, .woff2 se considera HLS
                // Esto es necesario porque Cuevana usa extensiones no estándar
                if (urlLower.includes('.m3u8') || 
                    urlLower.includes('.txt') || 
                    urlLower.includes('.woff2') ||
                    /\.(ts|m4s)(\?|$)/i.test(url)) {
                  const filename = url.substring(url.lastIndexOf('/') + 1);
                  playerLogger.log(`🎯 [TYPE-DETECT] HLS detectado: ${filename}`);
              ...
              const sourceConfig: any = {
                src: (() => {
                  ...
                  // Si es una URL relativa (empieza con /api/), devolverla tal cual
                  if (typeof BASE_STREAM_URL === 'string' && BASE_STREAM_URL.startsWith('/api/')) {
                    console.log('✅ [PROXY-DEBUG] Proxy local detectado (/api/), SIN re-proxy:', BASE_STREAM_URL.substring(0, 60));
              ...
              // ⚡ USAR HLS.JS PARA STREAMS HLS (MUCHO MÁS RÁPIDO)
              const isHLS = videoType === 'application/x-mpegURL';
              
              // CAMBIO: Usar VHS nativo de Video.js en lugar de HLS.js custom
              // VHS es más estable y no tiene problemas de BUFFER_FLUSHING
              // HLS.js custom causaba BUFFER_FLUSHING constante → eliminado
              // ❌ HLS.js CUSTOM ELIMINADO (~240 líneas removidas)
              // Causaba BUFFER_FLUSHING constante → ahora usamos VHS nativo
              if (false) {} else if (isHLS && videoRef.current?.canPlayType('application/vnd.apple.mpegurl')) {
                // Safari nativo soporta HLS
                playerLogger.log('🍎 [NATIVE-HLS] Usando soporte nativo de Safari para HLS');
                player.src(sourceConfig);
              } else {
                // Usar Video.js VHS nativo (más estable que HLS.js custom)
                playerLogger.log('🎬 [VIDEO.JS-VHS] Usando VHS nativo de Video.js');
                
                // Optimizar VHS para carga rápida y reproducción suave
                if (player.tech_ && (player.tech_ as any).vhs) {
                  const vhs = (player.tech_ as any).vhs;
                  playerLogger.log('✅ [VHS] Configurando optimizaciones...');
                  
                  // Configuración optimizada para VHS
                  vhs.options_ = {
                    ...vhs.options_,
                    // Buffer settings para carga rápida
                    experimentalBufferBasedABR: false,
                    handlePartialData: true,
                    // Aumentar bandwidth estimate para forzar mejor calidad
                    bandwidth: 10000000, // 10 Mbps inicial
                    // Buffer agresivo para inicio rápido
                    maxPlaylistRetries: 10,
                    segmentRequestTimeout: 20000,
                  };
                  
                  playerLogger.log('✅ [VHS] Optimizaciones aplicadas');
                }
                ...
                player.one('loadstart', () => {
                  if (player.tech_ && (player.tech_ as any).vhs) {
                    const vhs = (player.tech_ as any).vhs;
                    playerLogger.log('🎯 [VHS] Configurando post-loadstart...');
                    
                    // Forzar mejor calidad disponible
                    if (vhs.representations && vhs.representations().length > 0) {
                      const representations = vhs.representations();
                      representations.forEach((rep: any, i: number) => {
                        rep.enabled(i === representations.length - 1); // Solo habilitar la última (mejor)
                      });
                      playerLogger.log(`🔒 [VHS] Calidad bloqueada en la más alta`);
                    }
                  }
                });
              }
              ...
              // DESHABILITADO: Video.js qualityLevels - HLS.js ya maneja la calidad directamente
              // Dejar que HLS.js controle los niveles con autoLevelEnabled = false
              ...
              setTimeout(() => {
                ...
                // 🔧 IMPORTANTE: ELIMINAR el botón default de subtítulos y mostrar el real
                if (playerEl) {
                  // ELIMINAR completamente el botón default (SubsCapsButton) del DOM
                  const defaultSubsButton = playerEl.querySelector('.vjs-subs-caps-button');
                  if (defaultSubsButton) {
                    defaultSubsButton.remove(); // ELIMINAR, no solo ocultar
                    playerLogger.log('🗑️ [SUBTITLES] Botón default ELIMINADO del DOM (.vjs-subs-caps-button)');
                  }
                  
                  // Mostrar y activar el botón real (SubtitlesButton)
                  const realSubsButton = playerEl.querySelector('.vjs-subtitles-button');
                  if (realSubsButton) {
                    realSubsButton.classList.remove('vjs-hidden');
                    (realSubsButton as HTMLElement).style.display = '';
                    playerLogger.log('✅ [SUBTITLES] Botón real activado (.vjs-subtitles-button)');
                  }
                }
                ...
                setTimeout(() => {
                  if (playerEl) {
                    ...
                    // Volver a verificar botones de subtítulos después del timeout
                    const defaultSubsButton = playerEl.querySelector('.vjs-subs-caps-button');
                    if (defaultSubsButton) {
                      defaultSubsButton.remove(); // ELIMINAR del DOM, no solo ocultar
                      playerLogger.log('🗑️ [SUBTITLES] Botón default eliminado (verificación post-timeout)');
                    }
                    ...
                    // 🔍 DIAGNÓSTICO: Verificar qué elementos están bloqueando clicks
                    playerLogger.log('🔍 [CLICK-DEBUG] Diagnosticando elementos que pueden bloquear clicks...');
                    
                    // Verificar el elemento de video
                    const videoElement = playerEl.querySelector('video.vjs-tech');
                    if (videoElement) {
                      const videoStyle = window.getComputedStyle(videoElement);
                      playerLogger.log('🔍 [VIDEO] pointer-events:', videoStyle.pointerEvents, 'z-index:', videoStyle.zIndex);
                    } else {
                      playerLogger.warn('⚠️ [VIDEO] No se encontró elemento <video>');
                    }
                    
                    // Buscar todos los elementos con z-index alto que puedan estar tapando
                    const allElements = playerEl.querySelectorAll('*');
                    const highZIndexElements: any[] = [];
                    ...
                    if (highZIndexElements.length > 0) {
                      playerLogger.warn('⚠️ [CLICK-DEBUG] Elementos con z-index alto que pueden bloquear clicks:', highZIndexElements);
                    } else {
                      playerLogger.log('✅ [CLICK-DEBUG] No hay elementos con z-index alto bloqueando');
                    }
                    
                    // 🎯 SOLUCIÓN FINAL: Listeners manuales con userActions desactivado
                    // (userActions nativo de Video.js tiene conflictos con el botón de subtítulos)
                    playerLogger.log('🔧 [CLICK-MANUAL] Configurando listeners manuales (userActions desactivado)...');
                    
                    // Listener para click simple (play/pause)
                    const clickHandler = (e: Event) => {
                      const target = e.target as HTMLElement;
                      
                      // 🚫 NO procesar clicks en:
                      // - Control bar
                      // - Menús de Video.js
                      // - Modal de subtítulos (CRÍTICO!)
                      // - Overlay de pausa
                      // - Botón Volver
                      // - Botón Chromecast
                      if (
                        ...
                        target.closest('.vjs-subtitle-settings-modal') ||
                        target.closest('.pause-overlay') ||
                        target.closest('.vjs-back-button-overlay') ||
                        target.closest('.vjs-chromecast-button') ||
                        target.closest('.vjs-chromecast-button-overlay')
                      ) {
                        // Dejar que Video.js o React manejen estos clicks
                        return;
                      }
                      
                      // Solo en el área del video: bloquear Video.js y manejar nosotros
                      e.stopImmediatePropagation();
                      
                      playerLogger.log('👆 [CLICK-MANUAL] Click detectado en video');
                      if (player.paused()) {
                        player.play();
                        playerLogger.log('▶️ [CLICK-MANUAL] Play');
                      } else {
                        player.pause();
                        playerLogger.log('⏸️ [CLICK-MANUAL] Pause');
                      }
                    };
                    
                    // Listener para doble click (fullscreen) - usando evento nativo dblclick
                    const dblClickHandler = (e: Event) => {
                      const target = e.target as HTMLElement;
                      
                      // 🚫 NO procesar clicks en control bar, menús o modales
                      if (
                        target.closest('.vjs-control-bar') ||
                        target.closest('.vjs-menu') ||
                        target.closest('.vjs-subtitle-settings-modal') ||
                      ...
                      playerLogger.log('👆👆 [DBLCLICK-MANUAL] Doble click detectado');
                      if (player.isFullscreen()) {
                        player.exitFullscreen();
                        playerLogger.log('🪟 [DBLCLICK-MANUAL] Salir de fullscreen');
                      } else {
                        player.requestFullscreen();
                        playerLogger.log('🖥️ [DBLCLICK-MANUAL] Entrar a fullscreen');
                      }
                    };
                    
                    // Agregar listeners al playerEl con capture:true para ejecutar ANTES que Video.js
                    playerEl.addEventListener('click', clickHandler, true);
                    playerEl.addEventListener('dblclick', dblClickHandler, true); // ✅ Usar evento nativo dblclick
                    playerLogger.log('✅ [CLICK-MANUAL] Listeners manuales configurados');
                    
                    // Guardar para cleanup
                    (player as any)._clickHandler = clickHandler;
                    (player as any)._dblClickHandler = dblClickHandler;
                  }
                }, 1000);
            ...
            // Estilizar video element
                ...
                if (showEpisodeButton && onEpisodeButtonClick && controlBar) {
                  ...
                  if (controlBarEl) {
                    ...
                    if (fullscreenButton && fullscreenButton.parentNode === controlBarEl) {
                      try {
                        controlBarEl.insertBefore(episodeButton, fullscreenButton);
                      } catch (err) {
                        playerLogger.warn('⚠️ [PLAYER] Error al insertar botón de episodios, agregando al final:', err);
                    ...
                    if (!nextEpisodeButton) {
                      ...
                      if (fullscreenButton && fullscreenButton.parentNode === controlBarEl) {
                        try {
                          ...
                          playerLogger.warn('⚠️ [PLAYER] Error al insertar botón de siguiente episodio, agregando al final:', err);
                ...
                // Crear botón de audio (si hay custom stream o englishDub disponible)
                if (customStreamUrl || englishDubStreamUrl) {
                    ...
                    if (controlBarEl && !controlBarEl.querySelector('.vjs-audio-selector-button')) {
                      ...
                      if (fullscreenButton && fullscreenButton.parentNode === controlBarEl) {
                        try {
                          ...
                          playerLogger.warn('⚠️ [AUDIO-BUTTON] Error al insertar antes de Fullscreen, agregando al final:', err);
                          controlBarEl.appendChild(audioButton);
                        }
                      } else {
                        controlBarEl.appendChild(audioButton);
                        playerLogger.log('⚠️ [AUDIO-BUTTON] Fullscreen no encontrado o no es hijo directo, agregado al final');
                      }
                    }
                  }
                }
              ...
              setTimeout(() => {
                ...
                // ========== BOTÓN CHROMECAST (arriba a la derecha, igual que Volver) ==========
                // Crear botón manualmente (igual que el botón Volver)
                let chromecastButton = playerEl.querySelector('.vjs-chromecast-button-overlay') as HTMLButtonElement;
                if (!chromecastButton) {
                  chromecastButton = document.createElement('button') as HTMLButtonElement;
                  chromecastButton.className = 'vjs-chromecast-button-overlay';
                  chromecastButton.type = 'button';
                  chromecastButton.title = 'Cast';
                  chromecastButton.innerHTML = `
                  ...
                  `;
                  chromecastButton.style.cssText = `
                    position: absolute;
                    top: 20px;
                    right: 20px;
                    background: rgba(0, 0, 0, 0.6);
                  ...
                  chromecastButton.addEventListener('mouseenter', () => {
                    chromecastButton.style.backgroundColor = 'rgba(0, 0, 0, 0.7)';
                  });
                  chromecastButton.addEventListener('mouseleave', () => {
                    chromecastButton.style.backgroundColor = 'rgba(0, 0, 0, 0.6)';
                  });
                  
                  // Click handler: Acceder directamente a la API del plugin de Chromecast
                  chromecastButton.addEventListener('click', () => {
                    playerLogger.log('🎯 [CHROMECAST-OVERLAY] Click en botón manual');
                    
                    try {
                      // Opción 1: Buscar el botón real del plugin
                      const realChromecastBtn = playerEl.querySelector('.vjs-chromecast-button') as HTMLElement;
                      if (realChromecastBtn) {
                        playerLogger.log('✅ [CHROMECAST-OVERLAY] Clickeando botón real del plugin');
                        realChromecastBtn.click();
                        return;
                      }
                      
                      // Opción 2: Acceder directamente a la API de Chromecast
                      if ((player as any).chromecast) {
                        playerLogger.log('✅ [CHROMECAST-OVERLAY] Accediendo a API del plugin directamente');
                        
                        // Verificar si ya está conectado
                        if ((player as any).chromecast.isConnected && (player as any).chromecast.isConnected()) {
                          playerLogger.log('🔌 [CHROMECAST-OVERLAY] Ya conectado, desconectando...');
                          if ((player as any).chromecast.disconnect) {
                            (player as any).chromecast.disconnect();
                          }
                        } else {
                          playerLogger.log('📡 [CHROMECAST-OVERLAY] Abriendo diálogo de casting...');
                          // Abrir el diálogo de casting
                          if ((window as any).cast?.framework?.CastContext) {
                            const castContext = (window as any).cast.framework.CastContext.getInstance();
                            if (castContext && castContext.requestSession) {
                              castContext.requestSession();
                            }
                          }
                        }
                      } else {
                        playerLogger.warn('⚠️ [CHROMECAST-OVERLAY] Plugin de Chromecast no disponible');
                      }
                    } catch (error) {
                      playerLogger.error('❌ [CHROMECAST-OVERLAY] Error al manejar click:', error);
                    }
                  });
                  
                  playerEl.appendChild(chromecastButton);
                  playerLogger.log('✅ [CHROMECAST-OVERLAY] Botón Chromecast creado manualmente');
                }

                // Sincronizar visibilidad del botón Chromecast con los controles (igual que Volver)
                const syncChromecastButtonVisibility = () => {
                  const chromecastBtn = playerEl.querySelector('.vjs-chromecast-button-overlay') as HTMLElement;
                  if (!chromecastBtn) return;

                  // Verificar si el player está "useractive" (controles visibles) o pausado
                  const isActive = player.hasClass('vjs-user-active') || player.hasClass('vjs-paused');
                  chromecastBtn.style.opacity = isActive ? '1' : '0';
                  chromecastBtn.style.pointerEvents = isActive ? 'auto' : 'none';
                  chromecastBtn.style.transition = 'opacity 0.3s ease';
                  
                  // CRÍTICO: Ocultar el botón real del plugin si existe (solo queremos nuestro botón visible)
                  const realBtn = playerEl.querySelector('.vjs-chromecast-button:not(.vjs-chromecast-button-overlay)') as HTMLElement;
                  if (realBtn) {
                ...
                // Escuchar eventos de actividad del usuario
                player.on('useractive', syncChromecastButtonVisibility);
                player.on('userinactive', syncChromecastButtonVisibility);
                player.on('pause', syncChromecastButtonVisibility);
                player.on('play', syncChromecastButtonVisibility);
                
                // Sincronizar inicialmente
                setTimeout(syncChromecastButtonVisibility, 100);
                
                // Usar MutationObserver en lugar de polling para mejor performance
                const hideChromecastObserver = new MutationObserver(() => {
                  const realBtn = playerEl.querySelector('.vjs-chromecast-button:not(.vjs-chromecast-button-overlay)') as HTMLElement;
                  if (realBtn && realBtn.style.display !== 'none') {
                    realBtn.style.display = 'none';
                    playerLogger.log('🔇 [CHROMECAST-OVERLAY] Ocultando botón real del plugin (via observer)');
                  }
                });
                
                // Observar cambios en el DOM del control bar
                const controlBar = playerEl.querySelector('.vjs-control-bar');
                if (controlBar) {
                  hideChromecastObserver.observe(controlBar, { 
                    childList: true, 
                    subtree: true 
                  });
                }
                
                // Limpiar observer cuando el player se destruya
                player.on('dispose', () => {
                  hideChromecastObserver.disconnect();
                });
        ...
        // ⚡ OPTIMIZACIÓN: Llamar onReady() en loadedmetadata (más rápido que canplay)
        let hasCalledReady = false;
        player.on('loadedmetadata', () => {
          playerLogger.log('📊 [PLAYER] Metadata loaded');
          const duration = player.duration();
          if (duration && duration > 0) {
            playerLogger.log(`📊 [PLAYER] Duración: ${duration}s (${Math.floor(duration / 60)}min)`);
          }
          
          // Solo llamar onReady la primera vez (en loadedmetadata, más rápido)
          if (!hasCalledReady) {
            hasCalledReady = true;
        ...
        // Listener para canplay (solo para quitar el loading)
        player.on('canplay', () => {
        ...
        // DESHABILITADO: Stall recovery causaba BUFFER_FLUSHING y frame drops
        // Los eventos waiting/stalled son normales durante buffering inicial
        // HLS.js ya maneja recovery automáticamente
        if (isHlsStream) {
          playerLogger.log('🎬 [HLS] Stall recovery DESHABILITADO - HLS.js maneja recovery automáticamente');
        ...
        // Throttlear el evento progress para evitar micro-stuttering
        let lastProgressUpdate = 0;
        player.on('progress', () => {
          const now = Date.now();
          // Solo actualizar cada 2 segundos para evitar re-renders constantes
        ...
        const adjustSubtitlePosition = (controlBarVisible: boolean) => {
          ...
          // Ajustar subtítulos VTT nativos
          const textTrackDisplay = playerEl.querySelector('.vjs-text-track-display') as HTMLElement;
          if (textTrackDisplay) {
            playerLogger.log(`📐 [SUBTITLES-VTT] Ajustando posición - Barra visible: ${controlBarVisible}`);
            
            textTrackDisplay.style.zIndex = '3';
            
            if (controlBarVisible) {
              // Barra visible: movimiento instantáneo hacia arriba
              textTrackDisplay.style.transition = 'none';
              textTrackDisplay.style.transform = 'translateY(-120px)';
              textTrackDisplay.style.bottom = '2em';
            } else {
              // Barra oculta: transición suave de 500ms al volver
              textTrackDisplay.style.transition = 'transform 0.5s ease';
              textTrackDisplay.style.transform = 'translateY(0)';
              textTrackDisplay.style.bottom = '2em';
            }
          }

          // 🎨 Ajustar subtítulos ASS/SSA (contenedor completo)
          const assContainer = playerEl.querySelector('.vjs-ass-subtitle-container') as HTMLElement;
          if (assContainer) {
            playerLogger.log(`📐 [SUBTITLES-ASS] Ajustando posición - Barra visible: ${controlBarVisible}`);
            
            if (controlBarVisible) {
              // Barra visible: movimiento instantáneo hacia arriba
              assContainer.style.transition = 'none';
              assContainer.style.transform = 'translateY(-120px)';
            } else {
              // Barra oculta: transición suave de 500ms al volver
              assContainer.style.transition = 'transform 0.5s ease';
              assContainer.style.transform = 'translateY(0)';
            }
          }
        };
        ...
        player.textTracks().addEventListener('addtrack', () => {
          setTimeout(() => {
            ...
            // 🗑️ CRÍTICO: Eliminar botón default si se creó al agregar el track
            const defaultSubsButton = playerEl?.querySelector('.vjs-subs-caps-button');
            if (defaultSubsButton) {
              defaultSubsButton.remove();
              playerLogger.log('🗑️ [SUBTITLES] Botón default eliminado (post-addtrack)');
            }
          }, 100);
        });
        
        // 🛡️ DEFENSA FINAL: MutationObserver para eliminar el botón default si se crea dinámicamente
        const playerEl = player.el();
        if (playerEl) {
              ...
              if (mutation.type === 'childList') {
                mutation.addedNodes.forEach((node) => {
                  if (node instanceof HTMLElement) {
                    // Verificar si el nodo agregado es el botón default
                    if (node.classList.contains('vjs-subs-caps-button')) {
                      node.remove();
                      playerLogger.log('🗑️ [SUBTITLES] Botón default eliminado (MutationObserver - elemento completo)');
                    }
                    // También verificar si contiene el botón default
                    const defaultBtn = node.querySelector('.vjs-subs-caps-button');
                    if (defaultBtn) {
                      defaultBtn.remove();
                      playerLogger.log('🗑️ [SUBTITLES] Botón default eliminado (MutationObserver - descendiente)');
                    }
                  }
                });
              }
          ...
          playerLogger.log('🛡️ [SUBTITLES] MutationObserver configurado para eliminar botón default');
          
          // Guardar el observer para limpiarlo después
          (player as any)._subsButtonObserver = observer;
        }
      ...
      return () => {
        ...
        // ⚡ Limpiar hls.js primero
        if (hlsRef.current) {
          try {
            hlsRef.current.destroy();
            playerLogger.log('🧹 [HLS.JS] Instancia destruida');
          } catch (error) {
            playerLogger.warn('⚠️ [HLS.JS] Error destroying:', error);
          }
          hlsRef.current = null;
        }
        ...
        if (playerRef.current) {
          try {
            // Limpiar MutationObserver de botón de subtítulos
            if ((playerRef.current as any)._subsButtonObserver) {
              (playerRef.current as any)._subsButtonObserver.disconnect();
              playerLogger.log('🧹 [SUBTITLES] MutationObserver desconectado');
            }
            
            // Limpiar event listeners manuales de click (con capture:true)
            const playerEl = playerRef.current.el();
            if (playerEl && (playerRef.current as any)._clickHandler) {
              playerEl.removeEventListener('click', (playerRef.current as any)._clickHandler, true);
              playerEl.removeEventListener('dblclick', (playerRef.current as any)._dblClickHandler, true); // ✅ Usar dblclick
              playerLogger.log('🧹 [CLICK-MANUAL] Listeners manuales removidos');
            }
  ...
  // 🎧 useEffect para agregar/eliminar botón de audio dinámicamente cuando customStreamUrl o englishDubStreamUrl cambian
  useEffect(() => {
    const player = playerRef.current;
    if (!player) return;
    ...
    const existingButton = controlBarEl.querySelector('.vjs-audio-selector-button');

    if (customStreamUrl || englishDubStreamUrl) {
      // Si hay customStreamUrl O englishDubStreamUrl y NO existe el botón, crearlo
      if (!existingButton) {
        playerLogger.log('🎧 [AUDIO-BUTTON-DYNAMIC] Creando botón de audio dinámicamente...');
        ...
        // SVG de ecualizador (igual al original)
        audioButton.innerHTML = `
          <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 1.5em; height: 1.5em; display: block; margin: auto;"><g stroke-width="0"/><g stroke-linecap="round" stroke-linejoin="round"/><g fill="currentColor"><path d="M2 12.124C2 6.533 6.477 2 12 2s10 4.533 10 10.124v5.243c0 .817 0 1.378-.143 1.87a3.52 3.52 0 0 1-1.847 2.188c-.458.22-1.004.307-1.801.434l-.13.02a13 13 0 0 1-.727.105c-.209.02-.422.027-.64-.016a2.1 2.1 0 0 1-1.561-1.35 2.2 2.2 0 0 1-.116-.639c-.012-.204-.012-.452-.012-.742v-4.173c0-.425 0-.791.097-1.105a2.1 2.1 0 0 1 1.528-1.43c.316-.073.677-.044 1.096-.01l.093.007.11.01c.783.062 1.32.104 1.775.275q.481.181.883.487v-1.174c0-4.811-3.853-8.711-8.605-8.711s-8.605 3.9-8.605 8.711v1.174c.267-.203.563-.368.883-.487.455-.17.992-.213 1.775-.276l.11-.009.093-.007c.42-.034.78-.063 1.096.01a2.1 2.1 0 0 1 1.528 1.43c.098.314.097.68.097 1.105v4.172c0 .291 0 .54-.012.743-.012.213-.04.427-.116.638a2.1 2.1 0 0 1-1.56 1.35 2.2 2.2 0 0 1-.641.017c-.201-.02-.444-.059-.727-.104l-.13-.02c-.797-.128-1.344-.215-1.801-.436a3.52 3.52 0 0 1-1.847-2.188c-.118-.405-.139-.857-.142-1.461L2 17.58z"/><path fill-rule="evenodd" clip-rule="evenodd" d="M12 5.75a.75.75 0 0 1 .75.75v5a.75.75 0 1 1-1.5 0v-5a.75.75 0 0 1 .75-.75m3 1.5a.75.75 0 0 1 .75.75v2a.75.75 0 1 1-1.5 0V8a.75.75 0 0 1 .75-.75m-6 0a.75.75 0 0 1 .75.75v2a.75.75 0 0 1-1.5 0V8A.75.75 0 0 1 9 7.25"/></g></svg>
        `;

        // Aplicar estilos
        audioButton.style.cssText = 'width: 72px !important; height: 72px !important; min-width: 72px !important; min-height: 72px !important;';
        ...
        // Event listener
        audioButton.addEventListener('click', () => {
          playerLogger.log('🎧 [AUDIO-BUTTON-DYNAMIC] Click detectado');
          if (onToggleAudioMenu) {
            onToggleAudioMenu();
          }
        });

        // Buscar el grupo derecho y reorganizar botones en el orden correcto
        const rightGroup = controlBarEl.querySelector('.cpt-row-2-right');
        if (rightGroup) {
          // Insertar en el grupo derecho
          rightGroup.appendChild(audioButton);
          
          // Reorganizar todos los botones del grupo derecho en el orden correcto
          const subsBtn = controlBarEl.querySelector('.vjs-subtitles-button');
          const episodeBtn = controlBarEl.querySelector('.vjs-episode-selector-button');
          const nextEpisodeBtn = controlBarEl.querySelector('.vjs-next-episode-button');
          const fullscreenBtn = controlBarEl.querySelector('.vjs-fullscreen-control');
          
          // Orden: audio, subs, episodios, next, fullscreen
          const orderedButtons = [audioButton, subsBtn, episodeBtn, nextEpisodeBtn, fullscreenBtn].filter(Boolean);
          ...
          playerLogger.log('✅ [AUDIO-BUTTON-DYNAMIC] Botón de audio agregado y reorganizado en grupo derecho');
        } else {
          // Fallback: insertar antes del fullscreen
          const fullscreenButton = controlBarEl.querySelector('.vjs-fullscreen-control');
          if (fullscreenButton && fullscreenButton.parentNode === controlBarEl) {
            try {
              controlBarEl.insertBefore(audioButton, fullscreenButton);
              playerLogger.log('✅ [AUDIO-BUTTON-DYNAMIC] Botón de audio insertado antes de Fullscreen (fallback)');
            } catch (err) {
              playerLogger.warn('⚠️ [AUDIO-BUTTON-DYNAMIC] Error al insertar, agregando al final:', err);
              controlBarEl.appendChild(audioButton);
            }
          } else {
            controlBarEl.appendChild(audioButton);
            playerLogger.log('⚠️ [AUDIO-BUTTON-DYNAMIC] Agregado al final (fallback)');
          }
        }
      } else {
        // Botón ya existe, no hacer nada
      }
    } else {
      // Si NO hay customStreamUrl NI englishDubStreamUrl y existe el botón, eliminarlo
      if (existingButton) {
        playerLogger.log('🗑️ [AUDIO-BUTTON-DYNAMIC] Eliminando botón de audio...');
        existingButton.remove();
      }
    }
  }, [customStreamUrl, englishDubStreamUrl, onToggleAudioMenu]);
