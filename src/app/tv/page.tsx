...
export default function TVPage() {
  ...
  const selectedGenreName = genres.find(g => g.id === selectedGenre)?.name || 'Series';
  ...
  const fetchContentAndImages = async (pageNum: number, reset: boolean = false) => {
    setLoading(true);
    console.log('[TV DEBUG] Fetching genre:', selectedGenre, 'page:', pageNum);
    try {
      const response = await fetch(
        `/api/genre/${selectedGenre}?media_type=tv&page=${pageNum}`
      );
      
      console.log('[TV DEBUG] Response status:', response.status);
      
      if (response.ok) {
        const data = await response.json();
        console.log('[TV DEBUG] Data received:', data.results?.length, 'items');
        ...
        if (reset) {
          ...
          if (data.results.length > 0) {
            ...
            console.log('[TV DEBUG] Fetching images for:', featuredItem.id);
            
            const imagesResponse = await fetch(`/api/tv/${featuredItem.id}/images`);
            console.log('[TV DEBUG] Images response:', imagesResponse.status);
            
            if (imagesResponse.ok) {
              const imagesData = await imagesResponse.json();
              console.log('[TV DEBUG] Images data:', imagesData);
      ...
      console.error('[TV DEBUG] Error fetching:', error);
      logger.error('Error fetching TV shows:', error);
    } finally {
      console.log('[TV DEBUG] Loading finished');
      setLoading(false);
    }
  };
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        {totalResults > 0 && (
          <p className="text-white/50 text-sm text-center mb-8">
            {totalResults.toLocaleString()} series de {selectedGenreName}
          </p>
        )}
