...
export default function GenrePage() {
  ...
  const genreName = genreNames[genreId] || 'Género';
  ...
  const fetchContentAndImages = async (pageNum: number, reset: boolean = false) => {
    ...
    try {
      ...
      logger.error('Error fetching genre content:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      {/* Hero section estilo inicio con backdrop del primer item */}
      <div className="relative h-[80vh] overflow-hidden mt-24">
        {featuredItem ? (
          <>
            {/* Backdrop del item destacado */}
            <Image
              src={getImageUrl(featuredItem.backdrop_path || featuredItem.poster_path, 'original')}
              alt={getOriginalTitle(featuredItem)}
              fill
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/90 via-black/70 via-black/40 via-transparent to-transparent" />
            ...
            {/* Nombre del género - Esquina superior derecha */}
            ...
            <div className="absolute inset-0 flex items-center z-30">
              <div className="container mx-auto px-4">
                <div className="max-w-2xl">
                  {/* Logo o Título del item destacado */}
                  {getOriginalLogo(featuredImages) ? (
                    <div className="mb-4">
                      <img
                        src={getOriginalLogo(featuredImages)}
                        alt={getOriginalTitle(featuredItem)}
                        className="max-w-md w-full h-auto drop-shadow-2xl"
                        style={{ maxHeight: '150px', objectFit: 'contain', objectPosition: 'left' }}
                        loading="eager"
                      />
                    </div>
                  ) : (
                    <h2 className="text-4xl md:text-5xl font-bold text-white mb-4 drop-shadow-lg">
                      {getOriginalTitle(featuredItem)}
                    </h2>
                  )}
                  ...
                  {/* Overview */}
                  <p className="text-base md:text-lg text-gray-300 mb-6 line-clamp-3 drop-shadow-md max-w-xl">
                    {featuredItem.overview}
                  </p>
                  
                  {/* Botón ver ahora */}
                  <Link
                    href={mediaType === 'movie' ? `/watch?type=movie&id=${featuredItem.id}` : `/tv/${featuredItem.id}`}
                    className="inline-flex items-center gap-2 bg-white hover:bg-white/90 text-black px-8 py-3 rounded-lg font-semibold transition-all duration-200 shadow-lg hover:shadow-xl"
                  >
